<?php
    /*
        Today i watched a lecture about the concept of "clean code" and whilst i will not claim that this is perfect, this challenge is an attempt at making
        "clean code" with correctly named, simple methods that handle one purpose and one purpose only, as i said it is not perfect, but this is just an attempt!
    ...
    */
    ...
    function findNumericValuesInString($unfilteredString){
        return convertNumbersLargerThan3(array_filter(preg_split('/[^0-9]/', $unfilteredString), 'is_numeric'));
    }

    function convertNumbersLargerThan3($unconvertedArray) {
        return array_map(function($number) {
            return str_split($number, 3);
        }, $unconvertedArray);
    }

    function checkCubeNumber($number) {
        $isACubeNumber = 0;
        foreach(str_split($number) as $singleNum) {
            $isACubeNumber += $singleNum ** 3;
        }  
        return $number == $isACubeNumber;
    }

    function getCubicNumbersFromArray($array) {
    $cubeNumberArray = array();
    foreach($array as $number) {
            foreach($number as $index) {
            if (checkCubeNumber($index)) {
                array_push($cubeNumberArray, (string)(int) $index);
            } 
        }
    }
    ...
    function createReturnString($cubicNumbersArray) {
        return empty($cubicNumbersArray) ? "Unlucky" : implode($cubicNumbersArray, " ") . " " . array_sum($cubicNumbersArray) . " Lucky";
    }

    function isSumOfCubes($s) {
        $cubicNumbers = array_filter(getCubicNumbersFromArray(findNumericValuesInString($s)), 'is_numeric');
        return createReturnString($cubicNumbers);
    }